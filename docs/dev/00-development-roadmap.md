...
Phase 7: 页面消费层                                    ← 依赖 Phase 2 + 3
    ...
    │
Phase 8: MCP Server (@neuron-ui/mcp-server)             ← 依赖 Phase 3 + 7A + 7B
        将全部 AI 能力封装为标准 MCP 服务，
        任何 MCP 兼容客户端 (Claude/Cursor/Windsurf) 可直接消费
```
...
| 里程碑 | 阶段 | 交付物 | 验证方式 |
|--------|------|--------|---------|
| M0 - 工程可运行 | Phase 0 | `pnpm install && pnpm build` 通过, 组件脚手架脚本可用 | CI 绿色 + 脚本测试 |
| M1 - Token 可消费 | Phase 1 | CSS 变量 + TS 常量可 import | Storybook 基础页渲染 |
| M2 - P0 组件完成 | Phase 2 (P0) | 8 个核心组件可用 + 全链路验证通过 | Storybook + 单测 + json-render POC |
...
| M8 - Runtime 可用 | Phase 7A | `<NeuronPage>` 可渲染 Page Schema, POC 验证通过, 懒加载生效 | CRUD 页面动态渲染 + 性能达标 |
| M9 - CodeGen 可用 | Phase 7B | CLI 可生成 .tsx 源码 | 生成代码可编译运行 |
| M10 - MCP Server 可用 | Phase 8 | MCP Tools/Resources/Prompts 全部可调用 | Claude/Cursor 集成测试 |
...
- [Phase 8: MCP Server](./09-phase8-mcp-server.md)

> 注: 原全链路验收已合并至 Phase 6，不再单独设立。
...
Phase 8 (MCP Server) ──────────────────────────────────────────────────────────►  (依赖 Phase 3+7A+7B)
Phase 6 ────────────────────────────────────────────────────────────────────────────►  (全链路验收)
```
