...
将 shadcn/ui 作为底层原语，在其之上按照我们的暖灰色设计规范进行二次开发，产出一套 53 组件库。这些组件是后续目标 2（组件-接口映射）、目标 3（自动生成页面）、目标 4（拖拉拽编辑器）的基础。
...
| 覆盖率高      | 53 个组件中 ~80% 有 shadcn 对应，减少从零开发工作量 |
...
- **Zod Schema First**: 组件 Props 定义必须与 `neuronCatalog` 中的 Zod Schema 保持同步（Recommended: 从 Zod Schema 生成 TS 类型）
- **JSON Serializable**: 所有 Props 必须是 JSON 可序列化的（String, Number, Boolean），严禁 `ReactNode` 或 `Function` 类型（图标/渲染函数需通过字符串 ID 引用）
...
NButton/
├── NButton.tsx           # 组件实现
├── NButton.types.ts      # TypeScript Props 类型 (★ 需与 neuronCatalog Zod Schema 保持同步)
...
| -------------------- | ----------------- | ---------------- | --------------------- |
| 组件 Zod Schema      | 建立组件-接口映射 | 数据绑定时的校验 | 属性面板自动生成      |
...
| 1   | 53 个组件全部按设计规范开发完成                           |
| 2   | 所有 CSS 中零硬编码色值                                   |
| 3   | 每个组件有完整 TypeScript Props 类型且与 Zod Schema 同步  |
| 4   | 所有 Props 均为 JSON 可序列化类型 (无 ReactNode/Function) |
| 5   | 每个组件有 Storybook story                                |
| 6   | 单元测试覆盖率 ≥ 80%                                      |
